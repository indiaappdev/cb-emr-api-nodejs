 ...
 * 1. Added error handling with try-catch blocks
 * 2. Added browser configuration options for better performance
 * 3. Added page configuration for optimal PDF generation
 * 4. Implemented resource cleanup in finally block
 * 5. Added PDF configuration options for better quality
 * 6. Cached regex pattern for better performance
...
async function generatePDF(htmlPath, data) {
  ...
  try {
      ...
      // Use more efficient template replacement
      const htmlContent = htmlTemplate.replace(/{{\s*(\w+)\s*}}/g, (_, key) => 
          data[key] || ''); // Provide default empty string
      ...
      await page.addStyleTag({
          content: 'th { background-color: #ddd !important; color: black !important; }'
      });
