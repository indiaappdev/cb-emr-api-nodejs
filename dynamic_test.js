 ...
 *
 * @param {string} htmlPath - The path to the HTML template file.
 * @param {Object} data - An object containing dynamic data to inject into the template.
 * @return {Promise<void>} A Promise that resolves after the PDF is generated successfully.
 */
async function generatePDF(htmlPath, data) {
    outputPath = "styled_invoice.pdf"
    // Read the HTML template file
    const htmlTemplate = fs.readFileSync(htmlPath, 'utf8');

    // Inject dynamic data into the HTML template
    const htmlContent = htmlTemplate.replace(/{{\s*(\w+)\s*}}/g, (match, key) => data[key]);

    const browser = await puppeteer.launch();
    const page = await browser.newPage();

    // Set content of the page with the dynamic HTML
    await page.setContent(htmlContent);

    // Set the gray color explicitly for table headers
    ...
    try {
      ...
      for (const key in variables) {
        if (variables.hasOwnProperty(key)) {
          ...
          text = text.replace(replacer, variables[key]);
        }
      }
      return text;
    } catch (error) {
      // Log the error, assuming logger is set up for error reporting
      logger.error("mergeVariables:: error - " + error.stack);
    }
  };
